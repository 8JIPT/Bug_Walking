...
SoundHandle AudioSystem::PlaySound(const std::string& soundName, bool looping)
{
    ...
    if (freeChannel == -1){
        ...
        if (freeChannel == -1){
            freeChannel = 0;
            StopSound(mChannels[0]);
        }
    }
    ++mLastHandle;
    SoundHandle newHandle = mLastHandle;
    int loops = looping ? -1 : 0;
    if (Mix_PlayChannel(freeChannel, sound, loops) == -1){
        SDL_Log("Failed to play sound: %s", Mix_GetError());
        return SoundHandle::Invalid;
    }
    mChannels[freeChannel] = newHandle;
    mHandleMap[newHandle] = { soundName, freeChannel, looping, false };
